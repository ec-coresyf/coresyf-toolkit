...
import os 			# enables one to query file structures (e.g. seeing if a file is empty)
...
from coresyftools.tool import CoReSyFTool # enables the suite of Co-ReSyF tools to be used allowing this script to be run on hugh volumes of data.
...
class CoresyfOHMASeparabilityDecision(CoReSyFTool):
    ...
    def run(self, bindings): # this is a Method - n.b. "self" can stay as is, "bindings" does not need to be defined in your code
		...
		outfilePathName1 = outfile_Path + "/" + outfileName1
		outfileName2 = "2011_SeparabilityOutputsDetails.txt"
		outfilePathName2 = outfile_Path + "/" + outfileName2

		for infileName in os.listdir(infile_Path):
    		if infileName.endswith(".xlsx"): 
        		infilePathName = str(infile_Path + "/" + infileName)
