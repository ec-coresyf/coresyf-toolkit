...
class TestCoReSyFTool(TestCase):
    ...
    def test_zip_output(self):
        ...
        assert 'myoutput.zip' in files_
        archive = ZipFile('coresyftools/tests/tool6/myoutput.zip', 'r')
        self.assertGreater(len(archive.namelist()), 0)
        archive.close()
        os.remove('coresyftools/tests/tool6/myoutput.zip')
    ...
    def test_zip_output_from_tool(self):
        ...
        tool.execute(cmd)
        assert 'mydummyoutput.zip' in os.listdir('.')
        archive = ZipFile('mydummyoutput.zip', 'r')
        self.assertGreater(len(archive.namelist()), 0)
        archive.close()
        os.remove('f1')
        os.remove('mydummyoutput.zip')
        os.remove('mydummyoutput/f2')
        os.rmdir('mydummyoutput')
