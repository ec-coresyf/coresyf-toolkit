...
PathOut = "/home/riaa/Desktop/RA_SAR_Bathymetry/output/SAR_BathyOut_"+str(RunId)+"/FFT_img_outputs"
#newpath = r'./'+PathOut
...
for n,i in enumerate(Pontos):
    ...
    np.savez(npzOut, lons=lons, lats=lats, imgs=imgs)
...
### 
###
DepthOut=fileout+'Depth_'+str(RunId)+'.txt'
...
for n,i in enumerate(Pontos):           #### @TODO TO BE parallelized. Processed only for each existing FFT_.npz files. REMOVE For cycle
	fileoutTXT= fileout+"/"+str(RunId)+"_FFT_"+str(n+1)+".txt"
	...
	CDO_mean=CdoDirData[3]
	#DIR_mean=np.nanmean(CdoDirData[:,4])
	DIR_mean=CdoDirData[4]
	Depth=CSAR.SAR_LinearDepth(CDO_mean,W2_deep,Lmax)
	FDepthOut.write(" %s   %s   %s   %s   %s   %s   " % (n+1,CdoDirData[1],CdoDirData[2],CDO_mean,DIR_mean,Depth)+"\n")
FDepthOut.close()
