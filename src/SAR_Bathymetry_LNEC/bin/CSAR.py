...
def ContrastStretch2(img,limits=(0,3)):
	...
	img_new=((img - img.min())*((p_top-p_bottom) / (img.max() - img.min())))+ p_bottom
	...
	while True:
		...
		# if the 's' key is pressed, save the points
			...
			for n in xrange(Pt.shape[0]):
				...
				CoordsPtsAux[n,0],CoordsPtsAux[n,1]=LonVec[np.int(round(xaux))],LatVec[np.int(round(yaux))]
			...
			for i in xrange(Pt.shape[0]):
				cv2.circle(image,(np.int(round(xLine[i])),np.int(round(yLine[i]))),2,(0,0,255),2)
...
def LandMask(filein_ROI,dim, fileout="LandMask.tif", Pname="../output/SAR_BathyOut/"):
	...
	if Coords[:,0].min()>180.:
		xmin,xmax,ymin,ymax=Coords[:,0].min()-360., Coords[:,0].max()-360., Coords[:,1].min(), Coords[:,1].max()
		coords_flag=True
	else:
	...
	array2raster(Topo,Tlon,Tlat,EPSG_out=4326,fileout=Pname+"AA_TOPO_VFP.tif")
	...
	array2raster(th2,Tlon,Tlat,EPSG_out=4326,fileout=Pname+"AA_TOPO_VFP.tif")
	...
	cmd2="gdal_merge.py -n 0. -o "+fileout+" "+filein_ROI+" "+Pname+"AA_TOPO_VFP.tif"
	#print cmd1
	os.system(cmd1)
	#print cmd2
	os.system(cmd2)
	...
	ret,LandMask_inv=cv2.threshold(LandMask,127,255,cv2.THRESH_BINARY_INV)
...
def GetImgRes(filein,dim, path="../output/SAR_BathyOut/"):
	...
	if not os.path.isfile(fout):
		...
		EPSG_source=ProjLines.split("AUTHORITY")[-1].split(",")[1].split("\"")[1]
		#print EPSG_source, filein, fout
		if (dim[0]==0. and dim[1]==0.):
			os.system("gdalwarp -overwrite -srcnodata 0 -dstnodata 0 -s_srs EPSG:"+EPSG_source+" -t_srs EPSG:3763 "+filein+" "+fout)
		else:
			os.system("gdalwarp -overwrite -srcnodata 0 -dstnodata 0 -r average -ts "+str(dim[0])+" "+str(dim[1])+" -s_srs EPSG:"+EPSG_source+" -t_srs EPSG:3763 "+filein+" "+fout)
	else:
...
def SlantRangeGTiFF(filein,fileout,EPSG=4326):
	...
	output_raster.GetRasterBand(1).WriteArray(img3)   # Writes my array to the raster
	output_raster.GetRasterBand(1).SetNoDataValue(0)
	output_raster=None
	return None
...
def ReadSARImg(filein,ScaleFactor=1.,C_Stretch=False,SlantCorr=True,EPSG_flag="WGS84",Land=False, path="../output/SAR_BathyOut/", band=1):
	...
	if ScaleFactor!=1:
		...
		if not os.path.isfile(file_aux1):
			...
			os.system("gdalwarp -overwrite -srcnodata 0 -dstnodata 0 -r average -ts "+str(dim[0])+" "+str(dim[1])+" -t_srs EPSG:"+EPSG+" "+filein+" "+file_aux1)
		else:
			print file_aux1," Already Exists..."
	else:
		dim=[0,0]
		if not os.path.isfile(file_aux1):
			os.system("gdalwarp -overwrite -srcnodata 0 -dstnodata 0 -r average -t_srs EPSG:"+EPSG+" "+filein+" "+file_aux1)
		else:
			print file_aux1," Already Exists..."
	...
	if C_Stretch:
		if len(fileCS)==0:
			...
			array2raster(img,lon,lat,fileout=path+filein[:-4].split("/")[-1]+"_CS.tif")
			tf = time.clock()
		...
		os.system("cp "+file_aux1+" "+path+filein[:-4].split("/")[-1]+"_CS.tif")
	...
	if Land==True:
		MaskFinal=LandMask(fmask,dim,fileout=path+filein[:-4].split("/")[-1]+"_MaskFinal.tif",Pname=path)
		img= cv2.bitwise_and(img,img,mask = MaskFinal)
		array2raster(img,lon,lat,EPSG_out=4326,fileout=path+filein[:-4].split("/")[-1]+"_Masked.tif")
		mask=MaskFinal
...
def SAR_LinearDepth(CDO,W2,Lmax):
	...
	if CDO==np.nan:
		...
		depth= (CDO/(2.*np.pi)) *  np.arctanh((W2*CDO)/(2.*np.pi*9.8))
...
def FFT_SAR(img,lon,lat,fout,FFT_id,Lmax,res=1.25,Tmax=12.,verbose=False):
	...
	outputs:
			CDO and Direction
	"""
	center=[lon[lon.shape[0]/2,lon.shape[1]/2]  ,   lat[lat.shape[0]/2,lat.shape[1]/2]]
	...
	Fx=np.linspace(-magnitude.shape[1]/2.,magnitude.shape[1]/2.,magnitude.shape[1]+1)
	...
	maskO = sector_mask(img.shape,(img.shape[0]/2, img.shape[1]/2),pix_max_mask,(0,360))
	magnitude[maskO] = 0

	P2,max_freq=np.unravel_index(magnitude[:,magnitude.shape[0]/2:].argmax(),magnitude[:,magnitude.shape[0]/2:].shape),magnitude[:,magnitude.shape[0]/2:].argmax()
	...
	if p1!=p2:
		...
		lineOut=FFT_id+"   "+str(lon_aux2)+"   "+str(center[1])+"   "+str(CDO_new)+"   "+str(Dir)+"\n"
		fout.write(lineOut)
...
def GetFFTBox(Point,img,lon,lat,offset):
	img1=img[Point[1]-offset:Point[1]+offset,Point[0]-offset:Point[0]+offset]
	lon1=lon[Point[1]-offset:Point[1]+offset,Point[0]-offset:Point[0]+offset]
	lat1=lat[Point[1]-offset:Point[1]+offset,Point[0]-offset:Point[0]+offset]
...
def GetFFTBoxes(Point,img,lon,lat,offset,shift):
	...
	P_Grid=[(Point[0],Point[1]),
			(np.int(Point[0]-np.ceil(offset*shift)),	np.int(Point[1]+np.ceil(offset*shift))),
...
def PlotSARFFT(img,lon1,lat1,CDO,DIR,mag,deltaX,deltaY,p2_plot,Kscale,RMask,CDO_array,FFT_id,CDOMax=500.,zoom=25, dir="./FFT_outputs/"):
	...
	Kscale_labels=[Kscale_text[0],Kscale_text[zoom/2],Kscale_text[zoom],Kscale_text[zoom+(zoom/2)+1],Kscale_text[-1]]
	fig1=plt.figure(figsize=(15,10))
	ax1 = fig1.add_subplot(121)
	ax1.imshow(img,cmap=plt.cm.gray)
	plt.quiver(img.shape[1]/2,img.shape[0]/2,deltaX,deltaY,color='r')
	plt.title("Wavelength: "+str(round(CDO,2))+"m \nSwell Direction: "+str(round(DIR,2))+"$^\circ$")
	...
	ax2.set_xticks([offset-zoom,offset-(zoom/2),offset,offset+(zoom/2),offset+zoom])
	ax2.set_yticks([offset+zoom,offset+(zoom/2),offset,offset-(zoom/2),offset-zoom])
	...
	plt.text(offset-LambdaMAX, offset, str(np.int(CDOMax))+"m",fontsize=8,color="w",fontweight="bold")
	...
	for i in CDOs:
		...
		if Lradii[-1] <= zoom:
			cCDO=plt.Circle((offset,offset),Lradii[-1],color='w',fill=False)
			plt.text(offset-Lradii[-1], offset, str(np.int(i))+"m",fontsize=7,color="w",fontweight="bold")
			cCDOs.append(cCDO)
