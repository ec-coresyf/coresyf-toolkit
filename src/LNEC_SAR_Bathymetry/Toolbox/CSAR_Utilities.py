...
def ReadPointsfromGridFile(filename):            
    ...
    if '.npz' in filename:
		...
		Points = ReadPointsFromZippedShapefile(filename)
    else:
...
def ReadPointsFromZippedShapefile(filename):
    '''
    It opens a zip file containing a shapefile with points and returns an numpy
    ...
    try:
        ...
        print(e)
        sys.exit("Error. Unable to read Shapefile points in '%s'!" % filename)
...
def ReadZipShapefile(filename, temp_path="temp_input"):
    ...
    if not myzip.infolist():
        sys.exit("Input Zip file with grid shapefile '%s' is empty!" % filename)
    ...
    try:
        ...
        inDataSource = driver.Open(shapefile_shp, 0) # 0 means read-only. 1 means writable.
    except Exception as e:
        print(e)
        sys.exit("Error. Unable to open grid Shapefile '%s'!" % filename)
    return inDataSource
