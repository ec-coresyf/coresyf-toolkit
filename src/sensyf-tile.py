...
def tile_S1_data(xsize, ysize, offset, ulx, uly, lrx, lry, inputFilePath, dstDir, od):
    ...
    if (ulx != 0) or (uly != 0) or (lrx != 0) or (lry != 0):
        ...
        os.system(gpt_path + 'gpt.sh Subset ' + inputFilePath + ' -t ' + tmp_subset + ' "-PgeoRegion=POLYGON((' + str(ullon) + ' ' + str(ullat) + ', ' + str(
            lrlon) + ' ' + str(ullat) + ', ' + str(lrlon) + ' ' + str(lrlat) + ', ' + str(ullon) + ' ' + str(lrlat) + ', ' + str(ullon) + ' ' + str(ullat) + '))"')
    ...
    for i in range(num_tiles_row):
        ...
        for j in range(num_tiles_column):
            outputFilePath = outputDir + inputFileName + '_' + \
                str(regionX) + '_' + str(regionY) + '.tif'
            os.system(gpt_path + 'gpt.sh Subset ' + inputFilePath + ' -PregionX=' + str(regionX) + ' -PregionY=' + str(
                regionY) + ' -Pwidth=' + str(xsize + offset) + ' -Pheight=' + str(ysize + offset) + ' -t ' + outputFilePath)
...
def tile_default_od(xsize, ysize, offset, ulx, uly, lrx, lry, fileList, dstDir, od):
    ...
    for f in range(len(fileList)):
        ...
        if (extension.lower() == '.safe') and (os.path.isdir(inputFilePath)):
            tile_S1_data(xsize, ysize, offset, ulx, uly,
                         lrx, lry, inputFilePath, dstDir, od)

        elif (extension.lower() == '.tif') or (extension.lower() == '.tiff') or (extension == 'no_extension'):

            vector = get_new_off_coordinates(
                inputFilePath, inputProduct, xsize, ysize, offset, ulx, uly, lrx, lry, first_time)
        ...
        for i in range(int(vector[0])):
            for j in range(int(vector[1])):
                ...
                if extension == 'no_extension':
                    outputFilePath = outputDir + inputFileName + '_' + \
                        str(int(tile_xoff)) + '_' + str(int(tile_yoff))
                else:
                    outputFilePath = outputDir + inputFileName + '_' + \
                        str(int(tile_xoff)) + '_' + \
                        str(int(tile_yoff)) + extension
                os.system('gdal_merge.py -o ' + outputFilePath + ' -of GTiff -ul_lr ' + str(tile_xoff) +
                          ' ' + str(tile_yoff) + ' ' + str(tile_xend) + ' ' + str(tile_yend) + ' -q ' + inputFilePath)
        ...
        inputBandName, extension = os.path.splitext(
            os.path.basename(listInDir[b]))
        if (extension.lower() == '.tif') or (extension.lower() == '.tiff'):
            ...
            vector = get_new_off_coordinates(
                listInDir[b], inputProduct, xsize, ysize, offset, ulx, uly, lrx, lry, first_time)
            ...
            if vector[6] == 0:  # left_product variable 0 - ok, 1 - no tile
                ...
                outputFilePath = outputDir + inputBandName + '_' + \
                    str(int(tile_xoff)) + '_' + str(int(tile_yoff)) + extension
                os.system('gdal_merge.py -o ' + outputFilePath + ' -of GTiff -ul_lr ' + str(tile_xoff) +
                          ' ' + str(tile_yoff) + ' ' + str(tile_xend) + ' ' + str(tile_yend) + ' -q ' + listInDir[b])
            if vector[6] == 0:
        print 'Tiling process for product ' + inputProduct + ' complete.\n'
            else:
        if od == 0:
...
def tile_organized_by_tile(xsize, ysize, offset, ulx, uly, lrx, lry, fileList, dstDir):
    ...
    for f in range(len(fileList)):
        ...
        if (extension.lower() == '.safe') and (os.path.isdir(inputFilePath)):
            ...
            data_x_end = transform[0] + width * \
                transform[1] + height * transform[2]
            data_y_end = transform[3] + width * \
                transform[4] + height * transform[5]
        ...
        num_tiles_row = int(
            round((first_tile_yoff - last_tile_yoff) / rangeY + 1))
        num_tiles_column = int(
            round((last_tile_xoff - first_tile_xoff) / rangeX + 1))

        for i in range(num_tiles_row):
            for j in range(num_tiles_column):
                tile_xoff = first_tile_xoff + \
                    (j * rangeX) - (offset / 2) * pixel_size_x
                tile_yoff = first_tile_yoff - \
                    (i * rangeY) + (offset / 2) * pixel_size_y
                tile_xend = first_tile_xoff + \
                    (j + 1) * rangeX + (offset / 2) * pixel_size_x
                tile_yend = first_tile_yoff - \
                    (i + 1) * rangeY - (offset / 2) * pixel_size_y

                if os.path.exists(outputDir + str(int(tile_xoff)) + '_' + str(int(tile_yoff)) + '/'):
                    tileDir = outputDir + \
                        str(int(tile_xoff)) + '_' + str(int(tile_yoff)) + '/'
                else:
                    os.makedirs(outputDir + str(int(tile_xoff)) +
                                '_' + str(int(tile_yoff)) + '/')
                    tileDir = outputDir + \
                        str(int(tile_xoff)) + '_' + str(int(tile_yoff)) + '/'

                if extension == 'no_extension':
                    outputFilePath = tileDir + inputFileName + '_' + \
                        str(int(tile_xoff)) + '_' + str(int(tile_yoff))
                else:
                    outputFilePath = tileDir + inputFileName + '_' + \
                        str(int(tile_xoff)) + '_' + \
        ...
        if (extension.lower() == '.tif') or (extension.lower() == '.tiff'):
            ...
            if left_product == 0:
                ...
                outputFilePath = tileDir + inputBandName + '_' + \
                    str(int(tile_xoff)) + '_' + str(int(tile_yoff)) + extension
                os.system('gdal_merge.py -o ' + outputFilePath + ' -of GTiff -ul_lr ' + str(tile_xoff) +
                          ' ' + str(tile_yoff) + ' ' + str(tile_xend) + ' ' + str(tile_yend) + ' -q ' + listInDir[b])
...
def get_new_off_coordinates(inputFilePath, inputProduct, xsize, ysize, offset, ulx, uly, lrx, lry, first_time):
    ...
    if (ulx == 0) and (uly == 0) and (lrx == 0) and (lry == 0):
        ...
        transform = inputFile.GetGeoTransform()
        x_geo_in = transform[0] + borderx * \
            transform[1] + bordery * transform[2]
        y_geo_in = transform[3] + borderx * \
            transform[4] + bordery * transform[5]
    ...
    if len(args) < 5 and len(args) != 2:
        ...
        if len(directories) != 2:
            ...
            if len(fileList) == 0:
                ...
                tile_default_od(xsize, ysize, offset, ulx, uly,
                                lrx, lry, fileList, dstDir, od)
        elif ot == 1 and od == 0:
            fileList = glob.glob(directories[0] + "/*")
            if len(fileList) == 0:
                sys.exit("\nError: Input directory is empty.\n")
                ...
                dstDir = directories[1]
                tile_organized_by_tile(
                    xsize, ysize, offset, ulx, uly, lrx, lry, fileList, dstDir)
        elif ot == 0 and od == 1:
            fileList = glob.glob(directories[0] + "/*")
            if len(fileList) == 0:
                sys.exit("\nError: Input directory is empty.\n")
